...
describe ExportCsvService do
  ...
  let(:followers_csv) do
    ExportCsvService.new another_user.followed_last_month, User::CSV_ATTRIBUTES
  end
  ...
  it 'should include followed user in csv file' do
    expect(followers_csv.perform).to include(user.name)
  end
  ...
  context 'with old created (>1.month)' do
    ...
    it 'should not include micropost in csv file' do
      expect(micropost_csv.perform).to eq("Post,Date\n")
    end
  
    it 'should not include following in csv file' do
      expect(following_csv.perform).to eq("Full Name,Date\n")
    end
  
    it 'should not include follower in csv file' do
      expect(followers_csv.perform).to eq("Full Name,Date\n")
    end
  end
end
